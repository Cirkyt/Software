...
{   
  ...
  transfer_succeeded &= mpu_write_reg(ADDRESS_SIGNAL_PATH_RESET, reset_value);
...
//    if (mpu_read_reg(st.reg->temp, tmp, 2))
//        return -1;
//    raw = (tmp[0] << 8) | tmp[1];
...
int mpu_set_compass_sample_rate(unsigned short rate)
{
    ...
    if (mpu_write_reg(st.reg->s4_ctrl, div))
        return -1;
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    for (ii = 0; ii < packet_count; ii++) {
        ...
        if (mpu_read_reg(st.reg->fifo_r_w, data, MAX_PACKET_LENGTH))
            return -1;
...
int mpu_verify_product_id(void)
{
  ...
  if (mpu_read_reg(ADDRESS_WHO_AM_I, &who_am_i, 1))
  {
...
int mpu_read_reg(uint8_t reg, uint8_t *destination, uint8_t length)
{
  ...
  transfer_succeeded = twi_master_transfer(m_device_address, &reg, 1, TWI_DONT_ISSUE_STOP);
  transfer_succeeded &= twi_master_transfer(m_device_address|TWI_READ_BIT, destination, length, TWI_ISSUE_STOP);
  return transfer_succeeded;
}
