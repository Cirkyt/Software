...
#define CFG_ANDROID_ORIENT_INT  (1706)
#define FCFG_7                  (1076)
#define FCFG_6                  (1128)
...
int dmp_set_gyro_bias(long *bias)
{
    ...
    return mpu_write_mem(D_HOST_NO_MOT, 12, regs);
}
...
int dmp_set_orient_axes(unsigned char axis)
{
    ...
    return mpu_write_mem(D_1_74, 1, &axis);
}
...
#define PI_180  (3.14159f / 180.f)
    ...
    return mpu_write_mem(D_1_232, 4, tmp);
}
...
int dmp_set_orient_time(unsigned short time)
{
    ...
    tmp[0] = time >> 8;
    tmp[1] = time & 0xFF;
    return mpu_write_mem(D_1_250, 2 ,tmp);
}
...
int dmp_enable_feature(unsigned char mask)
{
    ...
    if (mask & (DMP_FEATURE_TAP | DMP_FEATURE_ORIENT))
        tmp[0] = DINA20;
    else
        tmp[0] = 0xD8;
    ...
    if (mask & DMP_FEATURE_ORIENT) {
        tmp[0] = 0xF8;
        tmp[1] = 0xF8;
        mpu_write_mem(CFG_ORIENT_IRQ_1, 2, tmp);
    ...
    /* In this DMP version, gyro and accel are always included in the FIFO. */
    dmp.packet_length = 12;
    if (mask & (DMP_FEATURE_LP_QUAT | DMP_FEATURE_6X_LP_QUAT))
        dmp.packet_length += 16;
    if (mask & (DMP_FEATURE_ORIENT | DMP_FEATURE_TAP))
        dmp.packet_length += 4;
...
int dmp_set_interrupt_mode(unsigned char mode)
{
    ...
    if (mpu_write_mem(CFG_FIFO_ON_EVENT, 12, tmp))
        return -1;
    ...
    tmp[6] = 0xDF;
    tmp[7] = 0xDF;
    return mpu_write_mem(CFG_6, 8, tmp);
}
    ...
    unsigned long *timestamp, short *sensors, unsigned char *more)
{
     ...
     */
    if (dmp.feature_mask & (DMP_FEATURE_ORIENT | DMP_FEATURE_TAP))
